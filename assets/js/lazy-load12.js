...
async function loadSection(id, endpoint) {
    ...
    try {
        const baseUrl = typeof LANDING_BASE_URL !== 'undefined' ? LANDING_BASE_URL : (window.location.origin + window.location.pathname.substring(0, window.location.pathname.lastIndexOf('/')));
    ...
    form.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (messageDiv) {
                ...
                if (result.success) {
                    messageDiv.className = 'text-green-700 bg-green-100 border border-green-200 text-sm mb-4 p-3 rounded fade-in';
                    messageDiv.textContent = result.message || 'Thank you for subscribing!';
                    emailInput.value = '';
                } else {
                    messageDiv.className = 'text-red-700 bg-red-100 border border-red-200 text-sm mb-4 p-3 rounded fade-in';
