...
function initLazyLoading() {
  ...
  const observer = new IntersectionObserver(
    ...
    { rootMargin: "200px" },
  ); // Reduced margin from 400px to 200px for better initial performance
      ...
      if (messageDiv) {
        ...
        if (result.success) {
          messageDiv.className =
            "text-green-700 bg-green-100 border border-green-200 text-sm mb-4 p-3 rounded";
          messageDiv.textContent =
            result.message || "Thank you for subscribing!";
          emailInput.value = ""; // Clear input
        } else {
          messageDiv.className =
            "text-red-700 bg-red-100 border border-red-200 text-sm mb-4 p-3 rounded";
          messageDiv.textContent =
...
function setupCustomSlider(sliderElement, prevBtnId, nextBtnId) {
  ...
  const getMetrics = (force = false) => {
    ...
    const visibleCount =
      Math.floor((wrapper.offsetWidth + gap) / (itemWidth + gap)) || 1;
