...
function initializeWishlistButtons() {
  document.querySelectorAll(".wishlist-btn").forEach((btn) => {
    ...
    if (productId && isInWishlist(productId)) {
      if (isCardButton) {
        // Use CSS variables via wishlist-active class
        ...
        if (icon) {
          ...
          // Add text-black only if not explicitly styled?
          // Let's keep it simple for text buttons for now.
        }
      }
      ...
      if (isCardButton) {
        ...
        btn.classList.remove("bg-black", "text-white", "wishlist-active");
        if (icon) {
...
async function refreshWishlist() {
  try {
    ...
    const response = await fetch(
      baseUrl + "/api/wishlist.php?t=" + new Date().getTime(),
      {
        method: "GET",
        headers: {
...
async function toggleWishlist(productId, button) {
  try {
        ...
        .forEach((btn) => {
          ...
          if (active) {
            ...
            if (isCardButton) {
              ...
              if (icon) icon.className = "fas fa-heart mr-1";
            }
            if (tooltip) {
              tooltip.textContent = removeText;
            } else if (
              btn.textContent.trim().toLowerCase().includes("wishlist")
            ) {
              if (icon) {
                btn.innerHTML = "";
                btn.appendChild(icon);
    ...
    if (data.success && Array.isArray(data.wishlist)) {
      ...
      const message = currentlyInWishlist
...
async function removeFromWishlist(productId) {
  try {
    const response = await fetch(
      (typeof BASE_URL !== "undefined" ? BASE_URL : "") + "/api/wishlist.php",
      {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
